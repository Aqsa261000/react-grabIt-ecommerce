  ...
  return (
    <div className="grid grid-rows-[1fr_min-content] grid-cols-2 pr-5 md:pr-0 md:grid-cols-[15%_1fr_40%] w-screen h-full gap-5 bg-gray-100">
      ...
      <div className="flex flex-col gap-2 p-5 col-span-2 md:col-span-1">
        <h1 className="font-bold text-3xl line-clamp-2 w-auto">
          {singleProduct.title}
        </h1>
        <p className=" text-[15px] line-clamp-3 w-auto">
          {singleProduct.description}
        </p>
        <div className="flex gap-4 items-center">
          <form className="flex items-center gap-5">
            <button
              className="cursor-pointer bg-[#ffae00]  px-4 py-2 rounded text-lg"
              onClick={(e) => {
                e.preventDefault();
          ...
          <p className="text-lg">Rs. {singleProduct?.price * quantity}</p>
        </div>
        <p className=" text-lg w-max p-5 rounded-md text-center bg-gray-300">
